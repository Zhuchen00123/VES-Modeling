"""Applying a recommendation solution to caller-provided data (R17).
...
hidden ratings the only successful status is ``produced_unverified`` and no
RMSE/MAE is ever produced.
"""
...
def apply_recommendation_solution(
    solution: str | Path,
    public_dir: Path,
    *,
    ...
    if not run_result.succeeded:
        ...
        try:
            ...
            validate_predictions(
                ...
                key_columns=(
                    contract.user_id_column,
                    contract.item_id_column,
                ),
            )
    ...
    summary = ApplyRecommendationResult(
        status=status,
        run_id=run_id,
        run_dir=run_dir,
