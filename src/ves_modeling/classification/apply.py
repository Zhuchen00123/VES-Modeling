"""Applying a classification solution to caller-provided data (R9).
...
accuracy/F1/AUROC (or any claimed quality metric) is ever produced.
"""
...
class ApplyClassificationResult:
...
def apply_classification_solution(
    solution: str | Path,
    public_dir: Path,
    *,
    ...
    config and provenance JSON files sit next to it.  No quality metric is
    ...
    summary = ApplyClassificationResult(
        status=status,
        run_id=run_id,
        run_dir=run_dir,
