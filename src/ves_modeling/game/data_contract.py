"""Discrete-time LQ zero-sum game data contract (R31).

``problem.json`` is the complete public instance (no hidden truth): version,
A (n x n finite), B (n x m control), C (n x p disturbance), Q (n x n
symmetric PSD), R (m x m symmetric PD), S (p x p symmetric PD disturbance
penalty), x0 (n finite), horizon N (int >= 2) and optional ``terminal_weight``
Q_N (n x n symmetric PSD, default Q).
...
class GameDataContract:
    """Canonical public LQ game instance (never host secrets)."""
...
def validate_game_data(public_dir: Path) -> GameDataContract:
    ...
    for key in ("A", "B", "C", "Q", "R", "S", "x0", "horizon"):
    ...
    m = int(B.shape[1])
    C = _finite_matrix(problem["C"], "C")
    if C.ndim != 2 or C.shape[0] != n or C.shape[1] < 1:
        raise ValueError("C must be an n x p matrix with p >= 1")
    p = int(C.shape[1])
    ...
    S = _finite_matrix(problem["S"], "S")
    _require_square(S, "S")
    _require_symmetric(S, "S")
    _require_pd(S, "S")
    if S.shape[0] != p:
...
def validate_control(
    payload: dict[str, Any], contract: GameDataContract
) -> np.ndarray:
