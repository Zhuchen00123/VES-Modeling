"""Candidate generators for the game slice."""
...
def _draft_prompt(*, n: int, m: int, p: int, horizon: int) -> str:
    lines = [
        (
            "You are solving a discrete-time LQ zero-sum differential game "
            "(minimax control vs disturbance)."
        ),
        "",
        "Available files:",
        "/data/problem.json",
        "",
        (
            f"state dimension: {n}. control dimension: {m}. disturbance "
            f"dimension: {p}. horizon: {horizon}."
        ),
        "",
        "You must create one complete Python program. The program must:",
        "1. Load /data/problem.json (A, B, C, Q, R, S, x0, horizon).",
        (
            "2. Produce a minimax control sequence u_0..u_{N-1} (e.g. via "
            "the game Riccati recursion)."
        ),
        ...
        (
            ...
            "dynamics with the worst-case disturbance and recomputes the "
            "total cost."
        ),
