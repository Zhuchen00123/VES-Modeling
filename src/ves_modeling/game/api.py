...
from ves_modeling.game.diagnostics import sha256_text, write_run_json
from ves_modeling.game.generator import (
    LlmClient,
    LLMGameGenerator,
    MockGameGenerator,
)
from ves_modeling.game.problem import build_game_problem
from ves_modeling.game.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "game"
...
class GameSearchResult:
    """Host-verified game search outcome."""
...
def run_game_search(
    ...
    *,
    ...
    """Search an LQ game problem and return the host-verified best."""
    ...
    problem = build_game_problem(public_dir, dataset_name=dataset_name)
    ...
    if generator == "mock":
        candidate = MockGameGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "game_riccati.py"
    ...
    result_object = GameSearchResult(
        run_id=run_id,
