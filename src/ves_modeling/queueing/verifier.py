"""Host-computed queueing metrics; candidate self-reports are ignored."""
...
from ves_modeling.queueing.data_contract import validate_solution


class QueueingVerifier:
    """EvidenceVerifier for queueing simulation solution artifacts."""
    ...
    def verify(
        ...
        if not isinstance(context, QueueingVerificationContext):
            raise TypeError(
                "QueueingVerifier requires QueueingVerificationContext"
            )
        ...
        for value in metrics:
            if not np.isfinite(value):
                raise ValueError("queueing metrics must be finite")
        return Evidence(
            observations=(
                Observation(
