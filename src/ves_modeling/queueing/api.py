"""Stable Queueing search API (R19)."""
...
from ves_modeling.queueing.data_contract import validate_queueing_data
from ves_modeling.queueing.diagnostics import sha256_text, write_run_json
from ves_modeling.queueing.generator import (
    LlmClient,
    LLMQueueingGenerator,
    MockQueueingGenerator,
)
from ves_modeling.queueing.problem import build_queueing_problem
from ves_modeling.queueing.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "queueing"
...
class QueueingSearchResult:
    """Host-verified queueing search outcome."""
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "queueing"


def run_queueing_search(
    public_dir: Path,
    *,
    drafts: int = 2,
    improves: int = 3,
    ...
    """Search a queueing simulation problem and return the host-verified
    best estimate."""
    ...
    problem = build_queueing_problem(
        public_dir, dataset_name=dataset_name
    )
    ...
    if generator == "mock":
        candidate = MockQueueingGenerator(
            fixture_dir, kind=contract.kind
        )
        ...
        if fallback_code is None:
            fallback_name = (
                "des_mm1.py" if contract.kind == "mm1" else "des_mmc.py"
            )
        ...
        candidate = LLMQueueingGenerator(
            llm_client,
    ...
    result_object = QueueingSearchResult(
        run_id=run_id,
