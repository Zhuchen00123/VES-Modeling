"""Stable Change-point search API (R25)."""
...
from ves_modeling.changepoint.data_contract import validate_changepoint_data
from ves_modeling.changepoint.diagnostics import sha256_text, write_run_json
from ves_modeling.changepoint.generator import (
    FIXED_INTERVAL_FALLBACK,
    LLMChangepointGenerator,
    LlmClient,
    MockChangepointGenerator,
)
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "changepoint"
)
...
class ChangepointSearchResult:
    """Host-verified change-point detection search outcome."""
    ...
    def to_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "best_mean_distance": self.best_mean_distance,
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "changepoint"


def run_changepoint_search(
    public_dir: Path,
    host_dir: Path,
    *,
    drafts: int = 2,
    ...
    """Search a change-point task and return the host-verified best."""
    ...
    problem = build_changepoint_problem(
        public_dir,
        host_dir,
    ...
    if generator == "mock":
        candidate = MockChangepointGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_code = FIXED_INTERVAL_FALLBACK
        llm_client = client or OpenAICompatibleClient()
        candidate = LLMChangepointGenerator(
            llm_client,
            fallback_code=fallback_code,
            n_test=contract.test_rows,
        )
    ...
    best_precision = (
        _observation(best_evidence, "precision") if best_evidence else None
    )
    best_recall = (
        _observation(best_evidence, "recall") if best_evidence else None
    )
    best_f1 = _observation(best_evidence, "f1") if best_evidence else None
    best_mean_distance = (
        _observation(best_evidence, "mean_distance")
        if best_evidence
        else None
    )
    ...
    result_object = ChangepointSearchResult(
        run_id=run_id,
        dataset_name=dataset_name,
        generator=generator,
        status=status,
        ...
        best_f1=best_f1,
        best_mean_distance=best_mean_distance,
    ...
    provenance = {
        ...
        "inputs": {
            ...
            "host": file_hashes(
                host_dir, ("hidden_test_changepoints.csv",)
            ),
        },
        "best": (
            {
