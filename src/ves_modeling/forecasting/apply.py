"""Applying a forecasting solution to caller-provided data (R8).
...
RMSE/MAE/SMAPE (or any claimed quality metric) is ever produced.
"""
...
def apply_forecasting_solution(
    solution: str | Path,
    public_dir: Path,
    *,
    ...
    config and provenance JSON files sit next to it.  No RMSE/MAE/SMAPE is
    produced because official test labels are not part of an apply call.
    """
    code = (
    ...
    if not run_result.succeeded:
        ...
        try:
            ...
            validate_predictions(
                ...
                test_keys=(
                    contract.test_keys if contract.row_order == "key" else None
                ),
                key_columns=(
                    contract.series_id_column,
    ...
    summary = ApplyForecastingResult(
        status=status,
        run_id=run_id,
        run_dir=run_dir,
