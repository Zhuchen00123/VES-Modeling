"""Candidate generators for the SIR slice."""
...
def _draft_prompt(*, quantity: str, n: int) -> str:
    lines = [
        "You are simulating an SIR epidemic model.",
        "",
        "Available files:",
        "/data/problem.json",
        "",
        f"quantity: {quantity}. population: {n}.",
        "",
        "You must create one complete Python program. The program must:",
        (
            "1. Load /data/problem.json (beta, gamma, N, i0, r0, t_end, "
            "quantity)."
        ),
        (
            "2. Simulate the SIR ODE (e.g. deterministic RK4 or stochastic "
            "Gillespie with replications) and compute the requested "
            "quantity."
        ),
        ...
        (
            ...
            "verifier independently solves the SIR equations and computes "
            "the error metrics."
        ),
...
class LLMSirGenerator:
    ...
    def draft(self, problem: VerifiedProblem, index: int) -> str:
        return self._complete(
            _draft_prompt(quantity=self._quantity, n=self._n)
        )
