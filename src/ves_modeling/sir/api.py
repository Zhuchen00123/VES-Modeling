...
from ves_modeling.sir.diagnostics import sha256_text, write_run_json
from ves_modeling.sir.generator import (
    LlmClient,
    LLMSirGenerator,
    MockSirGenerator,
)
from ves_modeling.sir.problem import build_sir_problem
from ves_modeling.sir.schema import API_SCHEMA_VERSION

GeneratorKind = Literal["mock", "llm"]

_REPO_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "sir"
...
class SirSearchResult:
    """Host-verified SIR search outcome."""
...
def run_sir_search(
    ...
    *,
    ...
    """Search an SIR problem and return the host-verified best."""
    ...
    problem = build_sir_problem(public_dir, dataset_name=dataset_name)
    ...
    if generator == "mock":
        candidate = MockSirGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "sir_rk4.py"
