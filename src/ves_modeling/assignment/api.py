"""Stable Assignment/TSP search API (R22)."""
...
from ves_modeling.assignment.diagnostics import sha256_text, write_run_json
from ves_modeling.assignment.generator import (
    LLMAssignGenerator,
    LlmClient,
    MockAssignGenerator,
)
from ves_modeling.assignment.problem import build_assignment_problem
from ves_modeling.assignment.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "assignment"
)
...
class AssignSearchResult:
    """Host-verified assignment/TSP search outcome."""
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "assignment"


def run_assignment_search(
    public_dir: Path,
    *,
    drafts: int = 2,
    improves: int = 3,
    ...
    """Search an assignment/TSP problem and return the host-verified best."""
    ...
    if generator == "mock":
        ...
        if fallback_code is None:
            fallback_name = (
                "assignment_hungarian.py"
                if contract.problem_type == "assignment"
                else "tsp_nearest_2opt.py"
            )
    ...
    result_object = AssignSearchResult(
        run_id=run_id,
