...
from ves_modeling.markov.data_contract import validate_markov_data
from ves_modeling.markov.diagnostics import sha256_text, write_run_json
from ves_modeling.markov.generator import (
    LlmClient,
    LLMMarkovGenerator,
    MockMarkovGenerator,
)
from ves_modeling.markov.problem import build_markov_problem
from ves_modeling.markov.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "markov"
...
class MarkovSearchResult:
    """Host-verified Markov search outcome."""
...
def _preflight_public_files(public_dir: Path) -> None:
    file_hashes(public_dir, ("problem.json", "train.csv"), strict=True)


def _default_fixtures() -> Path:
    if _REPO_FIXTURES.is_dir():
        return _REPO_FIXTURES
    return Path.cwd() / "fixtures" / "markov"


def run_markov_search(
    public_dir: Path,
    host_dir: Path,
    *,
    drafts: int = 2,
    ...
    """Search a Markov estimation problem and return the host-verified best."""
    ...
    result_object = MarkovSearchResult(
        run_id=run_id,
        dataset_name=dataset_name,
        generator=generator,
        status=status,
