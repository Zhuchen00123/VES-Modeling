"""Stable Optimization search API (R10).

Mirrors the other vertical slices: one concrete ``run_optimization_search``
entry returning a host-verified result and persisting the whole search under
``workspace/<run_id>``.  The public problem.json is the complete instance, so
there is no host-only directory.
"""
...
from ves_modeling.optimization.generator import (
    LlmClient,
    LLMOptimizationGenerator,
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "optimization"
)
...
def _is_feasible(evidence: Any, tolerance: float) -> bool:
    if evidence is None:
        return False
    for name in (
        "max_bound_violation",
        ...
        value = _observation(evidence, name)
...
class OptimizationSearchResult:
    """Host-verified optimization search outcome.

    ``best_*`` facts come from the host verifier, never from candidate
    self-reports.  ``best_feasible`` is host-determined within the documented
    tolerance; global optimality is never claimed.
    """
    ...
    best_max_bound_violation: float | None
    best_max_constraint_violation: float | None
    best_integrality_violation: float | None
    tolerance: float
    rejected: int
    run_dir: Path
    records: tuple[Any, ...]
    ...
    def to_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "best_max_bound_violation": self.best_max_bound_violation,
            "best_max_constraint_violation": (
                self.best_max_constraint_violation
            ),
            "best_integrality_violation": self.best_integrality_violation,
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "optimization"
...
def run_optimization_search(
    ...
    *,
    ...
    """Search a bounded linear/MILP problem and return the host-verified best.

    ``public_dir`` must contain ``problem.json`` (the complete public
    instance).  There is no hidden host directory for optimization.
    """
    ...
    if generator == "mock":
        candidate = MockOptimizationGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "milp_solution.py"
        ...
        candidate = LLMOptimizationGenerator(
            llm_client,
            ...
            has_integer=any(
                variable_type in ("integer", "binary")
                for _name, variable_type, _lower, _upper in contract.variables
            ),
        )
    ...
    best_feasible = _is_feasible(best_evidence, tolerance)
    status = (
        "verified"
    ...
    best_objective = (
        _observation(best_evidence, "objective") if best_evidence else None
    )
    best_max_bound = (
        _observation(best_evidence, "max_bound_violation")
        if best_evidence
        else None
    )
    best_max_constraint = (
        _observation(best_evidence, "max_constraint_violation")
        if best_evidence
        else None
    )
    best_integrality = (
        _observation(best_evidence, "integrality_violation")
        if best_evidence
        else None
    )
    best_candidate_id = (
    ...
    result_object = OptimizationSearchResult(
        run_id=run_id,
        dataset_name=dataset_name,
        generator=generator,
        status=status,
        ...
        best_max_bound_violation=best_max_bound,
        best_max_constraint_violation=best_max_constraint,
        best_integrality_violation=best_integrality,
        tolerance=tolerance,
