"""Stable Network-SIR search API (R30)."""
...
from ves_modeling.networksir.diagnostics import sha256_text, write_run_json
from ves_modeling.networksir.generator import (
    LlmClient,
    LLMNetworkSirGenerator,
    MockNetworkSirGenerator,
)
from ves_modeling.networksir.problem import build_networksir_problem
from ves_modeling.networksir.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "networksir"
)
...
class NetworkSirSearchResult:
    """Host-verified network-SIR search outcome."""
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "networksir"


def run_networksir_search(
    public_dir: Path,
    *,
    drafts: int = 2,
    improves: int = 3,
    ...
    """Search a network-SIR problem and return the host-verified best."""
    ...
    problem = build_networksir_problem(public_dir, dataset_name=dataset_name)
    ...
    if generator == "mock":
        candidate = MockNetworkSirGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "networksir_stochastic.py"
        ...
        candidate = LLMNetworkSirGenerator(
            llm_client,
    ...
    result_object = NetworkSirSearchResult(
        run_id=run_id,
