"""Stable Clustering search API (R12)."""
...
from ves_modeling.clustering.data_contract import validate_clustering_data
from ves_modeling.clustering.diagnostics import sha256_text, write_run_json
from ves_modeling.clustering.generator import (
    LlmClient,
    LLMClusteringGenerator,
    MockClusteringGenerator,
)
from ves_modeling.clustering.problem import build_clustering_problem
from ves_modeling.clustering.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "clustering"
)
...
class ClusteringSearchResult:
    """Host-verified clustering search outcome."""
    ...
    def to_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "best_v_measure": self.best_v_measure,
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "clustering"
...
def run_clustering_search(
    ...
    *,
    ...
    """Search an unsupervised clustering task and return the host-verified
    best partition."""
    ...
    problem = build_clustering_problem(
        public_dir,
        host_dir,
    ...
    if generator == "mock":
        candidate = MockClusteringGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "spectral_fallback.py"
        ...
        candidate = LLMClusteringGenerator(
            llm_client,
    ...
    best_ari = _observation(best_evidence, "ari") if best_evidence else None
    best_nmi = _observation(best_evidence, "nmi") if best_evidence else None
    best_v_measure = (
        _observation(best_evidence, "v_measure") if best_evidence else None
    )
    best_silhouette = (
        _observation(best_evidence, "silhouette") if best_evidence else None
    )
    best_candidate_id = (
    ...
    result_object = ClusteringSearchResult(
        run_id=run_id,
        dataset_name=dataset_name,
        generator=generator,
        status=status,
