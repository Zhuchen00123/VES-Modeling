"""Applying a survival solution to caller-provided data (R21).
...
hidden outcomes the only successful status is ``produced_unverified`` and no
quality metric is ever produced.
"""
...
def apply_survival_solution(
    ...
    *,
    ...
    """Apply ``solution`` to ``public_dir`` (train + test_features)."""
    code = (
    ...
    if not run_result.succeeded:
        ...
        try:
            ...
            validate_predictions(
                ...
                test_ids=(
                    contract.test_ids if contract.row_order == "id" else None
                ),
