"""Stable Survival search API (R21)."""
...
from ves_modeling.survival.data_contract import validate_survival_data
from ves_modeling.survival.diagnostics import sha256_text, write_run_json
from ves_modeling.survival.generator import (
    LlmClient,
    LLMSurvivalGenerator,
    MockSurvivalGenerator,
)
from ves_modeling.survival.problem import build_survival_problem
from ves_modeling.survival.schema import API_SCHEMA_VERSION

GeneratorKind = Literal["mock", "llm"]
OutputKind = Literal["risk_score", "time"]

_REPO_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "survival"
...
class SurvivalSearchResult:
    """Host-verified survival search outcome."""
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "survival"
...
def run_survival_search(
    ...
    *,
    ...
    row_order: str = "input",
    output_kind: OutputKind = "risk_score",
) -> SurvivalSearchResult:
    """Search a survival task and return the host-verified best."""
    ...
    contract = validate_survival_data(
        public_dir,
    ...
    problem = build_survival_problem(
        public_dir,
        host_dir,
    ...
    if generator == "mock":
        candidate = MockSurvivalGenerator(
            fixture_dir, output_kind=output_kind
        )
        ...
        candidate = LLMSurvivalGenerator(
            llm_client,
    ...
    best_c_index = (
        _observation(best_evidence, "c_index") if best_evidence else None
    )
    best_mae = (
    ...
    result_object = SurvivalSearchResult(
        run_id=run_id,
        dataset_name=dataset_name,
        generator=generator,
        status=status,
    ...
    provenance = {
        ...
        "inputs": {
            ...
            "host": file_hashes(host_dir, ("hidden_test_outcomes.csv",)),
        },
        "best": (
            {
