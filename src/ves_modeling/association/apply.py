"""Applying an association solution to caller-provided data (R20).

Untrusted/LLM code is executed in the Docker sandbox by default (only
``train.csv`` is mounted, read-only); local execution requires an explicit
``trusted_code=True`` opt-in.  The apply status remains ``produced_unverified``
and no quality metric is produced.
"""
...
class ApplyAssociationResult:
    ...
    def to_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "rules": (
                self._relative_to_run_dir(self.rules_path)
...
def _load_rules_payload(artifact_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"rules.json is not valid JSON: {exc}") from None
    if not isinstance(data, dict):
        raise ValueError("rules.json root must be an object")
    return data


def apply_association_solution(
    solution: str | Path,
    public_dir: Path,
    *,
    ...
    """Apply ``solution`` to ``public_dir`` (train.csv)."""
    code = (
    ...
    artifact_path = candidate_dir / "rules.json"
    if not artifact_path.is_file():
        artifact_path = candidate_dir / "output" / "rules.json"
    ...
    if not run_result.succeeded:
        ...
        try:
            payload = _load_rules_payload(artifact_path)
            validate_rules(payload, contract)
            rules_sha256 = sha256_file(artifact_path)
    ...
    data_hashes = file_hashes(public_dir, ("train.csv",))
    ...
    summary = ApplyAssociationResult(
        status=status,
        run_id=run_id,
        run_dir=run_dir,
