"""Stable Bin Packing search API (R24)."""
...
from ves_modeling.binpacking.diagnostics import sha256_text, write_run_json
from ves_modeling.binpacking.generator import (
    LLMBinGenerator,
    LlmClient,
    MockBinGenerator,
)
from ves_modeling.binpacking.problem import build_binpacking_problem
from ves_modeling.binpacking.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "binpacking"
)
...
class BinSearchResult:
    """Host-verified bin packing search outcome."""
    ...
    def to_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "best_bin_count": self.best_bin_count,
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "binpacking"


def run_binpacking_search(
    public_dir: Path,
    *,
    drafts: int = 2,
    improves: int = 3,
    ...
    """Search a bin packing problem and return the host-verified best."""
    ...
    problem = build_binpacking_problem(
    ...
    if generator == "mock":
        candidate = MockBinGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "bin_ffd.py"
    ...
    best_bin_count = (
        _observation(best_evidence, "bin_count") if best_evidence else None
    )
    ...
    status = (
        ...
        and best_capacity_violation is not None
        and best_capacity_violation <= 0.0
        else "no_verified"
    )
    best_candidate_id = (
