"""Candidate generators for the bin packing slice."""
...
def _artifact_format_prompt() -> str:
    return (
        "{{\n"
        '  "assignment": [<bin index>, <bin index>, ...]\n'
        "}}\n\n"
        "One bin index per item in problem.json order; bin indices are "
        "non-negative integers and the used bins must be contiguous "
        "0..k-1 (no gaps)."
    )


def _draft_prompt(*, n_items: int, capacity: float) -> str:
    lines = [
        "You are solving a one-dimensional bin packing problem.",
        "",
        "Available files:",
        "/data/problem.json",
        "",
        f"items: {n_items}. capacity: {capacity}.",
        "",
        "You must create one complete Python program. The program must:",
        "1. Load /data/problem.json.",
        (
            "2. Pack all items into bins (e.g. First-Fit-Decreasing or "
            "Best-Fit) respecting the capacity."
        ),
        ...
        "",
        (
            "Do not report or rely on a self-computed bin count, "
            "feasibility, optimality or gap."
        ),
        "The host verifier independently recomputes the packing facts.",
...
def _improve_prompt(
    ...
    *,
    ...
    lines += [
        "",
        "Improve the executable solution (fewer bins, capacity respected).",
        "",
        (
        ...
        "",
        (
            "Do not report or rely on a self-computed bin count; the host "
            "independently recomputes the packing facts. Never claim global "
            "optimality."
        ),
        "",
        "Allowed libraries: numpy, pandas (pure Python is preferred).",
...
class LLMBinGenerator:
    ...
    def draft(self, problem: VerifiedProblem, index: int) -> str:
        return self._complete(
            _draft_prompt(
                n_items=self._n_items, capacity=self._capacity
            )
        )
