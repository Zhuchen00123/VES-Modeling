...
from ves_modeling.lqr.diagnostics import sha256_text, write_run_json
from ves_modeling.lqr.generator import (
    LlmClient,
    LLMLqrGenerator,
    MockLqrGenerator,
)
from ves_modeling.lqr.problem import build_lqr_problem
from ves_modeling.lqr.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "lqr"
...
class LqrSearchResult:
    """Host-verified LQR search outcome."""
    ...
    best_reference_optimal_cost: float | None
    rejected: int
    run_dir: Path
    records: tuple[Any, ...]
    ...
    def to_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "best_total_cost": self.best_total_cost,
            "best_reference_optimal_cost": self.best_reference_optimal_cost,
...
def run_lqr_search(
    ...
    *,
    ...
    """Search an LQR problem and return the host-verified best."""
    ...
    problem = build_lqr_problem(public_dir, dataset_name=dataset_name)
    ...
    if generator == "mock":
        candidate = MockLqrGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "lqr_riccati.py"
    ...
    best_total_cost = (
        _observation(best_evidence, "total_cost") if best_evidence else None
    )
    best_reference_optimal_cost = (
        _observation(best_evidence, "reference_optimal_cost")
        if best_evidence
        else None
    )
    ...
    result_object = LqrSearchResult(
        ...
        best_reference_optimal_cost=best_reference_optimal_cost,
