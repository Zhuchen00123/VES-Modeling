"""Applying an LQR solution to caller-provided data (R26).

Untrusted/LLM code is executed in the Docker sandbox by default (only
``problem.json`` is mounted, read-only); local execution requires an
explicit ``trusted_code=True`` opt-in.  The apply status remains
...
from ves_modeling.lqr.data_contract import validate_control, validate_lqr_data
    ...
    *,
    ...
    if not run_result.succeeded:
        ...
        try:
            ...
            validate_control(payload, contract)
