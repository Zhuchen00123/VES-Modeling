"""Applying a sequential-pattern solution to caller-provided data (R27).
...
``trusted_code=True`` opt-in.  The apply status remains
``produced_unverified`` and no quality metric is produced.
"""
...
def _load_solutions_payload(artifact_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"patterns.json is not valid JSON: {exc}") from None
    if not isinstance(data, dict):
        raise ValueError("patterns.json root must be an object")
    return data


def apply_seqpattern_solution(
    solution: str | Path,
    public_dir: Path,
    *,
    ...
    artifact_path = candidate_dir / "patterns.json"
    if not artifact_path.is_file():
        artifact_path = candidate_dir / "output" / "patterns.json"
    ...
    if not run_result.succeeded:
        ...
        try:
            ...
            validate_patterns(payload, event_set=contract.event_set)
