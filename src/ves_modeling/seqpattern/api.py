"""Stable Sequential-pattern search API (R27)."""
...
from ves_modeling.seqpattern.data_contract import validate_seqpattern_data
from ves_modeling.seqpattern.diagnostics import sha256_text, write_run_json
from ves_modeling.seqpattern.generator import (
    LlmClient,
    LLMSeqPatternGenerator,
    MockSeqPatternGenerator,
)
from ves_modeling.seqpattern.problem import build_seqpattern_problem
from ves_modeling.seqpattern.schema import API_SCHEMA_VERSION

GeneratorKind = Literal["mock", "llm"]

_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "seqpattern"
)
...
class SeqPatternSearchResult:
    """Host-verified sequential-pattern search outcome."""
    ...
    best_evaluable_pattern_count: float | None
    best_pattern_count: float | None
    rejected: int
    run_dir: Path
    records: tuple[Any, ...]
    ...
    def to_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "best_evaluable_pattern_count": (
                self.best_evaluable_pattern_count
            ),
            "best_pattern_count": self.best_pattern_count,
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "seqpattern"


def run_seqpattern_search(
    public_dir: Path,
    host_dir: Path,
    *,
    drafts: int = 2,
    ...
    """Search a sequential-pattern task and return the host-verified best."""
    ...
    problem = build_seqpattern_problem(
        public_dir, host_dir, dataset_name=dataset_name
    )
    ...
    if generator == "mock":
        candidate = MockSeqPatternGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "seq_prefixspan.py"
        ...
        candidate = LLMSeqPatternGenerator(
            llm_client,
            fallback_code=fallback_code,
            event_set_size=len(contract.event_set),
        )
    ...
    best_evaluable_pattern_count = (
        _observation(best_evidence, "evaluable_pattern_count")
        if best_evidence
        else None
    )
    best_pattern_count = (
        _observation(best_evidence, "pattern_count")
        if best_evidence
        else None
    )
    ...
    result_object = SeqPatternSearchResult(
        run_id=run_id,
        ...
        best_evaluable_pattern_count=best_evaluable_pattern_count,
        best_pattern_count=best_pattern_count,
    ...
    provenance = {
        ...
        "inputs": {
            ...
            "host": file_hashes(
                host_dir, ("hidden_test_sequences.csv",)
            ),
        },
        "best": (
            {
