...
from ves_modeling.ode.data_contract import validate_ode_data
from ves_modeling.ode.diagnostics import sha256_text, write_run_json
from ves_modeling.ode.generator import (
    LlmClient,
    LLMOdeGenerator,
    MockOdeGenerator,
)
from ves_modeling.ode.problem import build_ode_problem
from ves_modeling.ode.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "ode"
...
class OdeSearchResult:
    """Host-verified ODE search outcome."""
...
def run_ode_search(
    ...
    *,
    ...
    """Search an ODE modeling task and return the host-verified best."""
    ...
    if generator == "mock":
        candidate = MockOdeGenerator(fixture_dir)
        ...
        if fallback_code is None:
            fallback_path = fixture_dir / "numpy_fallback.py"
    ...
    provenance = {
        ...
        "inputs": {
            ...
            "host": file_hashes(host_dir, ("hidden_test_values.csv",)),
        },
        "best": (
            {
