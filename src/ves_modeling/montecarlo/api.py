"""Stable Monte Carlo search API (R15)."""
...
from ves_modeling.montecarlo.data_contract import validate_montecarlo_data
from ves_modeling.montecarlo.diagnostics import sha256_text, write_run_json
from ves_modeling.montecarlo.generator import (
    LlmClient,
    LLMMonteCarloGenerator,
    MockMonteCarloGenerator,
)
from ves_modeling.montecarlo.problem import build_montecarlo_problem
from ves_modeling.montecarlo.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "montecarlo"
)
...
class MonteCarloSearchResult:
    """Host-verified Monte Carlo search outcome."""
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "montecarlo"


def run_montecarlo_search(
    public_dir: Path,
    *,
    drafts: int = 2,
    improves: int = 3,
    ...
    """Search a Monte Carlo estimation problem and return the host-verified
    best estimate."""
    ...
    problem = build_montecarlo_problem(
    ...
    if generator == "mock":
        candidate = MockMonteCarloGenerator(
        ...
        candidate = LLMMonteCarloGenerator(
            llm_client,
    ...
    result_object = MonteCarloSearchResult(
        run_id=run_id,
