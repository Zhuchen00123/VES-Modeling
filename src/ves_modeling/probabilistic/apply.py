"""Applying a probabilistic solution to caller-provided data (R18).

Untrusted/LLM code is executed in the Docker sandbox by default (only
``problem.json`` and ``train.csv`` are mounted, read-only); local execution
requires an explicit ``trusted_code=True`` opt-in.  The apply status remains
...
class ApplyProbabilisticResult:
...
def apply_probabilistic_solution(
    solution: str | Path,
    public_dir: Path,
    *,
    ...
    """Apply ``solution`` to ``public_dir`` (problem.json + train.csv)."""
    code = (
    ...
    if not run_result.succeeded:
        ...
        try:
            ...
            validate_solution(payload, contract)
    ...
    data_hashes = file_hashes(public_dir, ("problem.json", "train.csv"))
    ...
    summary = ApplyProbabilisticResult(
        status=status,
        run_id=run_id,
        run_dir=run_dir,
