"""Host-computed probabilistic metrics; candidate self-reports are ignored."""
...
from ves_modeling.probabilistic.data_contract import validate_solution


class ProbabilisticVerifier:
    """EvidenceVerifier for parameter-estimation solution artifacts.

    The host computes absolute_error and relative_error against its exact
    reference (relative_error falls back to absolute_error when the
    reference is zero).  ``ci_coverage`` is an audit observation.  Candidate
    self-reported references/parameters are never read.
    """
    ...
    def verify(
        ...
        if not isinstance(context, ProbabilisticVerificationContext):
            raise TypeError(
                "ProbabilisticVerifier requires "
                "ProbabilisticVerificationContext"
            )
        ...
        for value in metrics:
            if not np.isfinite(value):
                raise ValueError("probabilistic metrics must be finite")
        return Evidence(
            observations=(
                Observation(
