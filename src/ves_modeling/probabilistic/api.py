"""Stable Probabilistic search API (R18)."""
...
from ves_modeling.probabilistic.diagnostics import sha256_text, write_run_json
from ves_modeling.probabilistic.generator import (
    LlmClient,
    LLMProbabilisticGenerator,
    MockProbabilisticGenerator,
)
from ves_modeling.probabilistic.problem import build_probabilistic_problem
from ves_modeling.probabilistic.schema import API_SCHEMA_VERSION
...
_REPO_FIXTURES = (
    Path(__file__).resolve().parents[3] / "fixtures" / "probabilistic"
)
...
class ProbabilisticSearchResult:
    """Host-verified probabilistic search outcome."""
...
def _preflight_public_files(public_dir: Path) -> None:
    file_hashes(
        public_dir, ("problem.json", "train.csv"), strict=True
    )
...
def _default_fixtures() -> Path:
    ...
    return Path.cwd() / "fixtures" / "probabilistic"


def run_probabilistic_search(
    public_dir: Path,
    host_dir: Path,
    *,
    drafts: int = 2,
    ...
    """Search a parameter-estimation problem and return the host-verified
    best estimate."""
    ...
    problem = build_probabilistic_problem(
        public_dir,
        host_dir,
    ...
    if generator == "mock":
        candidate = MockProbabilisticGenerator(
            fixture_dir, quantity=contract.quantity
        )
        ...
        candidate = LLMProbabilisticGenerator(
            llm_client,
    ...
    best_ci_coverage = (
        _observation(best_evidence, "ci_coverage") if best_evidence else None
    )
    best_candidate_id = (
    ...
    result_object = ProbabilisticSearchResult(
        run_id=run_id,
        dataset_name=dataset_name,
        generator=generator,
        status=status,
    ...
    provenance = {
        ...
        "inputs": {
            "public": file_hashes(
                public_dir, ("problem.json", "train.csv")
            ),
            "host": file_hashes(host_dir, ("hidden_parameters.json",)),
        },
        "best": (
            {
