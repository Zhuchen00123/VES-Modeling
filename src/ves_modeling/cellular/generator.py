"""Candidate generators for the cellular-automaton slice."""
...
def _draft_prompt(*, quantity: str, width: int, steps: int) -> str:
    lines = [
        (
            "You are simulating a one-dimensional elementary cellular "
            "automaton."
        ),
        "",
        "Available files:",
        "/data/problem.json",
        "",
        f"quantity: {quantity}. width: {width}. steps: {steps}.",
        "",
        "You must create one complete Python program. The program must:",
        (
            "1. Load /data/problem.json (rule, width, steps, initial, "
            "quantity)."
        ),
        (
            "2. Iterate the elementary CA rule with periodic boundary "
            "for the requested steps and compute the quantity."
        ),
        ...
        (
            ...
            "verifier independently iterates the exact rule and computes "
            "the error metrics."
        ),
