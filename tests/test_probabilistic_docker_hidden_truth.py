"""R18: hidden truth isolation in the Docker runner (real attack when available)."""
...
import pandas as pd
samples = pd.read_csv("/data/train.csv")["value"].to_numpy(dtype=float)
with open("/output/solution.json", "w", encoding="utf-8") as fh:
    json.dump({"estimate": float(samples.mean()), "probe": found}, fh)
'''
...
def docker_env(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    samples = rng.normal(3.0, 2.0, size=100)
    ...
    (host / "hidden_parameters.json").write_text(
        json.dumps({"mean": 3.0, "std": 2.0}), encoding="utf-8"
    )
...
def test_hidden_truth_attack_real_container(docker_env) -> None:
    """Real attack: candidate must not read hidden parameters; output verified."""
