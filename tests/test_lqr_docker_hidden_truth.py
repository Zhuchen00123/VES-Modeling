"""R26: read-only problem mount in the Docker runner (real attack)."""
...
for p in ["/data/train.csv", "/data/test_features.csv",
          "/data/hidden_test_changepoints.csv", "/data/hidden_parameters.json",
...
with open("/output/solution.json", "w", encoding="utf-8") as fh:
    json.dump({"control": [[-0.6], [-0.2]], "probe": found}, fh)
'''
...
def test_read_only_problem_mount_real_container(docker_env) -> None:
    """Real attack: only problem.json is visible; cost still recomputed."""
    public, workspace = docker_env
    if subprocess.run(
