"""R14: read-only problem mount in the Docker runner (real attack when available)."""
...
with open("/output/solution.json", "w", encoding="utf-8") as fh:
    json.dump({"path": [problem["source"], problem["target"]], "probe": found}, fh)
'''
...
def test_read_only_problem_mount_real_container(docker_env) -> None:
    ...
    assert values["total_weight"] == pytest.approx(5.0)
