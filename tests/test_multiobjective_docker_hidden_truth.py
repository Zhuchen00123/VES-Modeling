"""R16: read-only problem mount in the Docker runner (real attack when available)."""
...
for name, spec in problem["variables"].items():
    variables[name] = (spec["lower"] + spec["upper"]) / 2.0
with open("/output/solution.json", "w", encoding="utf-8") as fh:
    json.dump({"solutions": [{"variables": variables}], "probe": found}, fh)
'''
...
def test_read_only_problem_mount_real_container(docker_env) -> None:
    ...
    assert values["feasible_count"] == 1.0
