"""R17: hidden truth isolation in the Docker runner (real attack when available)."""
...
found = {}
for p in ["/data/hidden_test_ratings.csv", "/host/hidden_test_ratings.csv",
          "/workspace/hidden_test_ratings.csv", "/output/hidden_test_ratings.csv",
          "/readonly/hidden_test_ratings.csv", "/data/../host/hidden_test_ratings.csv"]:
...
def docker_env(tmp_path: Path) -> tuple[Path, Path, Path]:
    train = pd.DataFrame(
        {
            "user_id": ["u0"] * 4 + ["u1"] * 4,
            "item_id": ["a", "b", "c", "d", "a", "b", "c", "d"],
            "rating": [1.0, 2.0, 3.0, 4.0, 2.0, 3.0, 4.0, 5.0],
        }
    )
    ...
    hidden = pd.DataFrame(
        {"user_id": ["u0", "u1"], "item_id": ["e", "e"], "rating": [3.0, 3.0]}
    )
    ...
    hidden.to_csv(host / "hidden_test_ratings.csv", index=False)
...
def test_hidden_truth_attack_real_container(docker_env) -> None:
    """Real attack: candidate must not read hidden ratings; output verified."""
    ...
    problem = build_recommendation_problem(public, host)
    ...
    for key, value in probe.items():
        if key.startswith("ls:"):
            continue
        assert value is False, f"hidden ratings leaked at {key}: {value}"
