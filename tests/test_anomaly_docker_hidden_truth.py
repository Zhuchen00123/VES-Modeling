"""R13: hidden truth isolation in the Docker runner (real attack when available)."""
...
with open("/output/predictions.json", "w", encoding="utf-8") as fh:
    json.dump({"scores": scores, "probe": found}, fh)
'''
...
def docker_env(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    train = rng.normal(0.0, 1.0, size=(60, 4))
    test = np.vstack(
        [
            rng.normal(0.0, 1.0, size=(20, 4)),
            rng.normal(6.0, 1.0, size=(10, 4)),
        ]
    )
...
def test_hidden_truth_attack_real_container(docker_env) -> None:
    ...
    assert np.isfinite(values["average_precision"])
