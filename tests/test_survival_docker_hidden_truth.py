"""R21: hidden truth isolation in the Docker runner (real attack when available)."""
...
found = {}
for p in ["/data/hidden_test_outcomes.csv", "/host/hidden_test_outcomes.csv",
          "/workspace/hidden_test_outcomes.csv", "/output/hidden_test_outcomes.csv",
          "/readonly/hidden_test_outcomes.csv", "/data/../host/hidden_test_outcomes.csv"]:
...
scores = [float(index) for index in range(len(test))]
with open("/output/predictions.json", "w", encoding="utf-8") as fh:
    json.dump({"predictions": scores, "probe": found}, fh)
'''
...
def docker_env(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    times = 10.0 + 5.0 * x[:, 0] + rng.exponential(scale=5.0, size=40)
    events = (rng.random(40) < 0.6).astype(int)
    public = tmp_path / "public"
    host = tmp_path / "host"
    public.mkdir()
    host.mkdir()
    train = pd.DataFrame(x[:30], columns=["f0", "f1"])
    train["time"] = times[:30]
    train["event"] = events[:30]
    pd.DataFrame(x[30:], columns=["f0", "f1"]).to_csv(
        public / "test_features.csv", index=False
    )
    pd.DataFrame({"time": times[30:], "event": events[30:]}).to_csv(
...
def test_hidden_truth_attack_real_container(docker_env) -> None:
    """Real attack: hidden outcomes never mounted; predictions verified."""
    ...
    for key, value in probe.items():
        if key.startswith("ls:"):
            continue
        assert value is False, f"hidden outcomes leaked at {key}: {value}"
    ...
    assert np.isfinite(values["c_index"])
