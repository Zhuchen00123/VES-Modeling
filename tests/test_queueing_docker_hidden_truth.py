"""R19: read-only problem mount in the Docker runner (real attack when available)."""
...
for p in ["/data/hidden_parameters.json", "/host/hidden_parameters.json",
          "/data/hidden_test_ratings.csv", "/data/train.csv",
...
with open("/output/solution.json", "w", encoding="utf-8") as fh:
    json.dump(
        {"estimate": 0.5, "confidence_interval": [0.4, 0.6], "probe": found},
        fh,
    )
'''
