"""R12: hidden truth isolation in the Docker runner (real attack when available)."""
...
labels = [f"cluster_{index % 2}" for index in range(len(test))]
with open("/output/predictions.json", "w", encoding="utf-8") as fh:
    json.dump({"labels": labels, "probe": found}, fh)
'''
...
def docker_env(tmp_path: Path) -> tuple[Path, Path, Path]:
    X, y = make_blobs(
        n_samples=150, centers=3, n_features=4, random_state=3
    )
    train = pd.DataFrame(X[:120], columns=[f"f{i}" for i in range(4)])
    test_features = pd.DataFrame(X[120:], columns=[f"f{i}" for i in range(4)])
    hidden = pd.DataFrame({"label": [int(value) for value in y[120:]]})
