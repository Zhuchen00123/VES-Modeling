"""Trusted fixture candidate: low-resolution coarse-grid fallback.

Reads VES_DATA_DIR/problem.json (default /data) and writes
VES_OUTPUT_DIR/solution.json (default /output) with an approximate CA
quantity from a spatially downsampled grid.  Self-reported references are
never trusted by the host.
"""
...
persistent = coarse.copy()
for _ in range(steps):
    coarse = step(coarse)
    persistent = persistent & coarse

width_c = len(coarse)
if quantity == "final_density":
    estimate = float(np.count_nonzero(coarse)) / width_c
elif quantity == "cell_state":
    estimate = float(coarse[index // factor % width_c])
else:
    estimate = float(np.count_nonzero(persistent)) * factor
