"""Trusted fixture candidate: sklearn KMeans clustering.

Fits KMeans on train with a simple inertia-elbow k choice and assigns every
test row.  Writes VES_OUTPUT_DIR/predictions.json (input mode without an id
column, id mode with one).  Self-reported scores are never trusted by the
host.
"""
...
model = KMeans(n_clusters=k, n_init=10, random_state=42).fit(train_matrix)
labels = [f"cluster_{int(value)}" for value in model.predict(test_matrix)]
